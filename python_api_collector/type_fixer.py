...
def get_candidates_for_drama_fix(cursor, limit=20):
    """ 'Drama' 타입이 아닌 작품 목록을 가져옵니다. """
...
def ask_gemini_is_drama(work_data):
    """ Gemini에게 작품이 서사 중심의 '드라마'인지 물어봅니다. """
...
def main():
    ...
    PROCESS_LIMIT = 30
    candidates = get_candidates_for_drama_fix(cursor, limit=PROCESS_LIMIT)
    ...
    try:
        for work in candidates:
            ...
            if judgment == 'Drama':
                # work_type_mapping에 Drama 타입 추가
                ...
                # INSERT IGNORE는 UNIQUE KEY 제약조건 위반 시 오류 없이 무시함
