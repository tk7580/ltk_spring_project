...
DROP TABLE IF EXISTS `WORK`; -- 테이블명 대문자 유지 (원본 DDL 기준)
DROP TABLE IF EXISTS `board`;
DROP TABLE IF EXISTS `series`;
DROP TABLE IF EXISTS `MEMBER`;
-- 테이블명 대문자 유지 (원본 DDL 기준)
...
(                                                                                    -- 테이블명 대문자 유지 (원본 DDL 기준)
    ...
    `NAME`         CHAR(20)  NOT NULL COMMENT '회원 이름',                               -- 컬럼명 대문자 유지 (원본 DDL 기준)
...
(
    ...
    `DESCRIPTION`   TEXT COMMENT '시리즈 전체 개요/줄거리', -- 컬럼명 대문자 유지 (원본 DDL 기준)
...
(
    ...
    `CODE`       CHAR(50) NOT NULL UNIQUE COMMENT '게시판 코드 (예: notice, free, QnA)', -- 컬럼명 대문자 유지 (원본 DDL 기준)
    `NAME`       CHAR(20) NOT NULL UNIQUE COMMENT '게시판 이름',                        -- 컬럼명 대문자 유지 (원본 DDL 기준)
...
(                                                                                                         -- 테이블명 대문자 유지 (원본 DDL 기준)
    ...
    `TYPE`             VARCHAR(50)  NOT NULL COMMENT '작품 유형 (Movie, Drama, Webtoon, Comic, Animation 등)', -- 컬럼명 대문자 유지 (원본 DDL 기준)
    `releaseDate`      DATE COMMENT '최초 공개일 (영화 개봉일, 웹툰 연재 시작일, 애니 방영 시작일 등)',
    `releaseSequence`  INT(10) UNSIGNED NOT NULL DEFAULT 0 COMMENT '시리즈 내 발매/공개 순서',
    `timelineSequence` INT(10) UNSIGNED NOT NULL DEFAULT 0 COMMENT '시리즈 내 내부 시간 흐름 순서',
    `isCompleted`      TINYINT(1) UNSIGNED DEFAULT 0 COMMENT '개별 작품 완결 여부 (0=미완결, 1=완결)',
    `DESCRIPTION`      TEXT COMMENT '개별 작품 상세 설명/시놉시스',                                                   -- 컬럼명 대문자 유지 (원본 DDL 기준)
...
(
    ...
    FOREIGN KEY (`memberId`) REFERENCES `MEMBER` (`id`) ON DELETE CASCADE, -- 참조 테이블명 대문자 `MEMBER`로 수정
    FOREIGN KEY (`seriesId`) REFERENCES `series` (`id`) ON DELETE CASCADE
);
...
(
    ...
    `relId`        INT(10) NOT NULL COMMENT '관련 데이터 번호',                  -- 게시글 ID 또는 댓글 ID이므로 INT(10) 유지 가능 (단, UNSIGNED 여부는 해당 테이블 ID 정의에 따름)
...
(
    ...
    `relId`             INT(10) UNSIGNED NOT NULL COMMENT '관련 게시물 (article) 번호', -- article.id가 INT(10) UNSIGNED이므로 유지
    `parentId`          INT(10) UNSIGNED COMMENT '부모 댓글의 ID (최상위 댓글은 NULL)',     -- reply.id가 INT(10) UNSIGNED이므로 유지
...
(
    ...
    `workId`         INT(10) UNSIGNED NOT NULL COMMENT '해당 콘텐츠가 속한 개별 작품 ID', -- WORK.id가 INT(10) UNSIGNED이므로 유지
    ...
    FOREIGN KEY (`workId`) REFERENCES `WORK` (`id`) ON DELETE CASCADE         -- 참조 테이블명 대문자 `WORK`로 수정
);